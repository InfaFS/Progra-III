...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	@Override
	public boolean eliminar (T i) {
		NodoGenerico<T> n = this.inicio;
		NodoGenerico<T> ant = null;
		while ((n != null) && (!n.getDato().equals(i))) {
			ant = n;
			n = n.getSiguiente();
		}
		if (n == null)
	...
	public ListaGenericaEnlazada<T> invertir(){
		...
		ListaGenericaEnlazada<T> nuevaLista= new ListaGenericaEnlazada<T>();
		this.comenzar();
		invGenerico(nuevaLista);
		
		return nuevaLista;
		
	}
	
	public ListaGenericaEnlazada<T> invGenerico(ListaGenericaEnlazada<T> ListaInvertida){
			
		if (this.fin()) {
			return ListaInvertida;
		}
		
		ListaInvertida.agregarInicio(this.proximo());
